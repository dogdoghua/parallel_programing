...
int main(int argc, char **argv)
{
    ...
    int additer=0;      // for send receive iteration count
    for(int i=world_size; i>1; i/=2){
        additer++;
    }// i=8 => add=3
    ...
    if(world_size>1){
        MPI_Status status;
        for(int k=0, cnt=2; k<additer; k++, cnt*=2){
            // receiver
            if(world_rank % cnt == 0){
                global_toss_in_circle = local_toss_in_circle;
                MPI_Recv(&local_toss_in_circle, 1, MPI_INT, world_rank + (cnt/2), 0, MPI_COMM_WORLD, &status);
                global_toss_in_circle += local_toss_in_circle;
                local_toss_in_circle = global_toss_in_circle;
            }

            // sender
            else if(world_rank % cnt == (cnt/2)){
                MPI_Send(&local_toss_in_circle, 1, MPI_INT, world_rank - (world_rank%cnt), 0, MPI_COMM_WORLD);
                break;
            }
        }
    }
